...
| 能力 | 场景 |
|------|-----------|
| `startup_bridge` | **新开 outside window 的第一优先入口**。先走真实海马体启动桥，再按 core/recent/diary/window 的轻量读取预算拿 startup recall |
...
### 主动调用
- **新开 outside window**：优先调用 `startup_bridge(scene="outside_daily_window")`，不要只靠像记得一样的说话方式开场
- **对话开头**：如果不需要完整启动桥，再无参数调用 `breath()`，看看有没有高权重的未解决记忆需要跟进
